...
Flutter 多端独立短剧应用，原名“短剧库 APP”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前源码及红果鉴 / 真果鉴两份 Android ARM64 Release 安装包均为 **0.2.16+22（功能未验证开发快照）**。

按用户 2026-09-21 的要求，暂停测试与回归。0.2.16 接入黄剧分类、分页在线搜索、详情与带签名 Cookie 的媒体请求，榜单图标移到顶部标题栏“排序与筛选”后面。本轮完成源码整理、收尾同步和两版 Android ARM64 Release 打包与包级检查；未运行静态分析、自动化测试、回归、设备 / 模拟器或真实站源验证。

| 编译方式 | 应用名称 | 可用站源 |
| --- | --- | --- |
| 默认，不加参数 | 红果鉴 | 仅红果 |
| 构建脚本加 `--all-sources` | 真果鉴 | 红果、黄豆、黄剧、黄果视频、黄果 AI、黄果旧版 |

全站源版首页提供“全部站源 / 红果 / 黄豆 / 黄剧 / 黄果”；全部站源只汇总当前用户获准的入口。黄剧作为独立站源；黄果统一汇总视频、AI 与旧 API 的分类和内容。六个实际接口保留各自的权限、分页、独立更新和健康检测，可在站源管理中分别维护。已有受限用户不会自动获得新增黄剧权限，管理员可在用户管理中勾选。
...
| 功能 | 操作 |
| --- | --- |
| 浏览 | 标题栏依次提供排序与筛选、榜单图标、多选下载和展开搜索；榜单紧跟排序，窄屏常显。分类保持独立一行，内容区左右滑动切类；支持默认、名称、自然季号、上线日期、热度、播放量排序及连载状态筛选。排序作用于已加载内容，未知值置后；“更新剧库”或下拉执行查新、历史续载与资料补齐，窄屏的更新入口位于“更多”，底部加载更多 |
| 搜索 | 红果合并官网与名称索引，补齐可识别的缺漏季；输入停顿 300 毫秒显示最多 10 条红果联想，正式名称查询独立使用 50 条索引记录。黄剧调用站源在线搜索，按真实页码和总数继续加载；单独搜索黄剧不调用红果联想。部分失败保留有效结果；最近 20 次搜索按用户保存并可清空。其余站源只筛选已加载短剧 |
| 推荐 | 仅红果站源的分类栏在“全部”后显示“推荐”，点选后直接在当前页浏览；可切换真人剧、漫剧、AI 剧，分别记住推荐位置，可刷新或继续推荐。其他站源与“全部站源”不显示此入口。公开推荐不连接官方账号 |
| 详情 | 上方浏览封面、资料、追剧状态与可展开简介；底部固定“立即播放 / 继续播放”和下载入口。选集每组 50 集，可切换范围、定位当前或输入集数跳转；宽屏与电视采用资料、选集分栏 |
...
| 下载 | 按剧分组搜索、筛选，标题栏进入多选 / 全选；支持批量暂停、继续、重试、删除及清理任务但保留视频。“更新本剧”补新增或缺失分集 |
...
| 黄剧（全站源版） | 热门、最新、接口分类、详情及可播放分集；签名 Cookie 接入在线播放、预加载和下载，待验证 | 站源在线搜索，支持继续加载 |
...
0.2.16 依据用户提供的 `黄剧.js` 接入 `https://huangju.net` 与 `https://api.huangju.net`。默认“全部”使用热门目录，另提供“最新”及接口实际返回的分类；目录和在线搜索使用响应中的分页信息，失败保留已有结果。详情按真实分集编号排序，跳过明确标为不可播放的分集，保留源站总集数与连载状态；剧目身份沿用脚本的 `slug-id`，标题、简介、地区、年份和评分按返回资料展示。
...
黄剧复用详情回写、站源更新、下载合集、追剧与备份权限。访客令牌和媒体 Cookie 不进入追剧记录、配置备份或局域网同步；推送播放由接收设备自行取流。局域网连接不再限制为五个站源，新版用扩展能力字段交换完整范围，同时保留旧版认识的站源字段；与旧版连接时只同步双方共有的站源，黄剧记录保留在支持它的设备上。上述接口与双端行为尚未集中验证，本轮没有请求站源图片或真实媒体。
...
倍速、画质、自动连播、下一集预加载和弹幕开关按本地用户保存，并纳入配置备份；新开播放器沿用偏好。临时 3 倍速不会改写偏好，切集、弹出菜单、失去焦点、转入后台或退出页面时结束临时倍速。关闭自动连播后，播放结束停在当前集；保存失败会提示重试。
...
0.2.12 默认开启下一集预加载，可在播放设置关闭。前台正常播放至少 2 秒、播放器报告缓冲余量至少 5 秒，且已进入本集后半段或最后 45 秒时，低优先级解析并预取下一集；不启动本地视频预取。关闭、主动切集、播放失败、进入后台或切换用户会取消在途工作并释放过期播放计划，正常连播可接用已就绪计划。

解析时限 15 秒，预取最多 6 秒、4 MiB、8 次请求；HLS 限制列表深度和前两段、约 8 秒媒体，MP4 只取开头并在有效范围及版本校验通过后复用。就绪计划最多保留 2 分钟，0.2.16 对黄剧等返回有效期的计划提前 5 秒停止复用；失败至少间隔 30 秒再试，避免无限预取或反复解析。

播放器优先从 mpv 实际可跳转缓存区间显示连续缓冲时长与范围；无法取得区间时只显示播放器报告的“已缓冲至”，不把单一终点当作连续范围。0.2.12 的实现曾完成静态与核心编译检查，0.2.16 的凭证有效期处理仅完成源码接入与格式整理；预取复用、网络切换和各平台缓存反馈待集中验证。
...
播放器在常规、全屏与 TV 控制区提供投放图标，提示文字为“推送”或“推送到［设备名］”。未连接时打开共用设备选择面板，已连接时直接开始准备；切换已连接目标可从同步页操作。窄屏优先保留推送和主要播放控制，次要选项在播放设置中；独立推送面板显示接收设备、剧名、准备 / 定位 / 成功或失败状态，并提供取消。
...
| 播放设置 | 倍速、画质、追剧、自动连播、预加载和弹幕开关；切画质保留进度和暂停状态，偏好按用户保存 |
| 首页返回 | 先返回发现，再清空搜索；发现且无搜索词时退出 |

支持专用播放 / 暂停、上一集、下一集按键。播放中无操作 5 秒后隐藏控制条，暂停和操作弹窗时保持可见。竖屏视频保持原比例，不裁剪填满电视。

## 视频画质增强与超分方案（2026-09-22 调研，待实施）

**可以在现有架构上实现。推荐先交付 Android / Windows 共用的 GPU 画质增强，再为 Windows 增加 NVIDIA / Intel 硬件超分；手机上的较重神经网络单独评估。** 目标是改善低清真人短剧在实际观看尺寸下的清晰度，同时保持连续播放、音画同步和可接受的发热。首期重点是 SDR 视频约 1.3–2 倍的放大，不以所有视频输出 4倍 / 4K 为目标。

本节是针对当前 **0.2.15+21** 播放链路的实施方案。当前 App **尚未接入超分，也没有超分按钮**；现有“清晰度”选择的是源站画质。2026-09-22 本轮完成源码和公开资料调研，只修改文档，没有运行 App 静态检查、构建、回归或设备测试。下文性能数字注明其原始测试条件，工程阈值是待验证的初始设计，不是本 App 的实测成绩。
...
| 已核实事实 | 对实施的要求 |
| --- | --- |
| 项目使用 `media_kit 1.2.6`、`media_kit_video 2.0.1`；`PlayerScreen` 直接创建默认 `VideoController`。当前没有超分状态、Shader 或相关偏好 | 沿用现有播放器，在 `lib/player_screen.dart`、播放偏好和设置菜单中接入统一增强控制器 |
| 已安装的 media_kit 初始化时设置 `scale=bilinear`、`dscale=bilinear`、`correct-downscaling=no`，App 没有覆盖 | 高质量缩放本身就是可实施的第一步，但应明确称为画质增强，不标成神经网络超分 |
...
| `media_kit_libs_windows_video 1.0.11` 的构建配置指向 `2023-09-24 / 652a1dd`；该提交的 `d3d11vpp` 没有超分缩放参数 | NVIDIA / Intel 缩放支持于 2024-08-20 的 PR #14698 合入。Windows 硬件超分必须更新并固定原生库，不能仅更新 Dart 参数。[SR05]、[SR06] |
...
| ArtCNN / Anime4K | 明确面向动漫，与真人皮肤和发丝的目标不同 | ArtCNN 文档区分实时 C 系列和较重的 R 系列；Anime4K 主要面向原生 1080P 动漫 | 仅作为以后独立“动漫增强”选项，不做真人短剧默认。[SR11]、[SR12] |
...
`d3d11vpp=scale=1.5:scaling-mode=nvidia:format=nv12` 仅是 **SDR、相应上下文已就绪时** 的参数示例；Intel 使用相应的 `intel` 模式。倍率由显示目标计算，滤镜需要自己的标签，卸载只移除本功能拥有的配置。未经能力确认不向当前 2023 年内核发送这组参数。mpv 文档明确指出，这些模式只启用相应驱动扩展，实际效果依赖硬件与驱动设置。[SR06]
...
首批未验证开发快照默认关闭增强。集中验证后，对通过的设备 / 后端组合开放“自动”；未知设备从保守基础档开始。判断设备时使用实际渲染后端与能力，不只根据 Android 版本、显卡品牌或旗舰机名称判断。

| 平台 / 条件 | 初始自动策略 |
| --- | --- |
| Android 手机 | 实际需要放大时，先用已通过的基础缩放 / RGB 轻量路线；神经网络初始候选限制输入约不超过 `540×960`、输出约不超过 2.1 百万像素、倍率不超过 2，再根据整条链路数据调整 |
...
| Android TV | 第二优先级；复用已通过的保守策略，不能因电视输出 4K 就默认启用 4倍或重型 CNN |
...
**拟定入口：播放器 → 播放设置 → 画质增强（超分）**，并在“清晰度”面板提供直达项。选项采用“关闭 / 自动 / 省电增强 / 清晰优先”，由设备选择具体后端；不在普通用户流程堆放模型名、Shader 路径和图形 API 名称。实际信息显示“轻量增强”“神经超分”或“已因发热降低增强”等，与用户选择的偏好分开。

提供独立“原画对比”按钮，切换同一播放器的增强状态，保持位置、倍速、音量、源画质和相同的显示尺寸，不新建第二个播放器。不要占用已有的画面长按 3倍速手势。字幕控件和 Flutter 弹幕在增强后叠加；片源本身烧录的字幕无法单独排除，需纳入质量验收。TV 入口和对比按钮均可被遥控焦点到达。
...
所有阶段目前均为待实施，不变更前面已有功能的验收状态。仍按先完整实现当前交付范围、后由用户安排集中验证的节奏推进；本轮不启动下面的测试。
...
| [SR06] mpv PR #14698 / 当前文档 | 缩放扩展于 2024 年合入；实际启用依赖硬件和驱动 |
...
| [SR16] NTIRE 2026 Mobile Real-World SR | 天玑 8400、小图速度测试、质量 / 速度综合排名与公开代码 |
...
0.2.16+22 已完成两版 Android ARM64 打包与安装包检查，ARM64 分包版本码为 2022。仓库没有 `key.properties`，两包按既有配置沿用 Android Debug 证书；证书指纹与本地 0.2.4+11 包一致，可覆盖同证书旧包，但不等同正式生产签名。此版本包含黄剧站源及榜单位置调整。Windows / iOS 完整应用仍待相应平台构建，本轮没有进行设备运行确认。

| 平台 | 包与状态 |
| --- | --- |
| Android 8.0+ 手机 | 0.2.16+22 提供两版 `arm64-v8a` APK；ARMv7 / x86_64 构建脚本保留，目前仅有历史版本安装包 |
...
| 当前版本 | 安装包 | 大小 |
| --- | --- | --- |
| 红果鉴，仅红果 | `dist/android/hongguojian-0.2.16+22-arm64-v8a.apk` | 30.9 MB |
| 真果鉴，全部站源 | `dist/android/zhenguojian-0.2.16+22-arm64-v8a.apk` | 30.9 MB |
...
以下对照 `../短剧库` 与红果专用版 `../果果剧库` 的功能和近期修复，基于 2026-09-21 的 0.2.6 审查，并更新至 App **0.2.16+22** 的源码实施状态。已有解析辅助代码不代表 App 已接入功能；未完成集中验证与平台验收的项目保持未勾选，标明实际实施与检查范围。同时记录 App 自身的数据与恢复边界缺陷。Android 手机和 Windows 同为首要平台，设备验收单独记录。

共 31 项：3 项沿用历史已完成标记，26 项已实现但待集中验证，1 项为黄果 AI 播放恢复的外部连接阻塞，1 项为已调研、待实施的视频画质增强与超分。0.2.12 已补入原清单最后六项可独立实施的功能及界面调整；0.2.13 接入新增 P3-2 / P3-3，2026-09-22 新增 P3-4 调研方案；0.2.16 接入 P1-9 黄剧站源。新增功能均保留未验证状态，功能范围与验收状态分别记录。
...
| P1-8 | [ ] 已实现，待验证：搜索完整性 | 0.2.9 红果合并官网及名称索引，按 ID 去重并保留同名不同 ID，补识别到的漏季；加入归一化、分词匹配、相关度和自然季号排序、最近 20 次搜索。联想 10 条与正式索引 50 条独立；总时限 40 秒、补漏季最多 32 次，保留部分成功结果，不虚构下一页。目录 / 搜索取消贯通 FFI，前台在途上限 16；0.2.16 新增黄剧在线分页搜索，其余站源仅筛选已加载内容 |
| P1-9 | [ ] 已实现，待验证：黄剧站源 | 0.2.16 依据用户提供的黄剧脚本接入访客授权、热门 / 最新 / 实际分类、分页目录与在线搜索、详情及真实分集；CloudFront 签名 Cookie 限定媒体来源，贯通播放、预加载、下载和健康检测。复用用户权限、资料回写与局域网共同站源协商。未进行真实接口、播放、下载、双端或平台验证，仅全站源版开放 |
...
| P2-4 | [ ] 已实现，待验证：下一集预加载与缓冲反馈 | 0.2.12 接入低优先级解析、最多 6 秒 / 4 MiB / 8 次请求的预取、HLS 前两段和 MP4 前缀复用；计划超时、失败退避、切集 / 后台取消及用户开关已接通。读取 mpv 实际可跳转区间显示连续缓冲，降级只显示缓冲终点；待验证取消、版本 / Range 校验、连播复用和平台反馈 |
...
| P2-10 | [ ] 已实现，待验证：播放偏好与全屏交互 | 0.2.9 保存画质、倍速和自动连播，按用户隔离并纳入备份；补全全屏选集 / 倍速 / 画质 / 设置及 Go 版长按和键盘规则，临时倍速在松手、切集、菜单、后台和失焦时复原。0.2.12 统一每组 50 集、范围切换 / 精确跳转 / 当前定位，紧凑布局整合切集和播放控制；待集中覆盖重开、重启、备份、布局及输入边界 |
...
| P3-4 | [ ] 已调研，待实施：视频画质增强与超分 | 2026-09-22 完成论文、文档、issues 与成熟源码调研。方案见本 README“视频画质增强与超分方案”：Android / Windows 共用尺寸适配与轻量 GPU 增强，Windows 增加硬件超分，手机 CNN 分阶段评估；包含拟定入口、实际生效判定、热降级和集中验收矩阵。当前没有超分实现、按钮或本 App 效果 / 性能测试结论 |
...
| 在线播放闭环 | 独立站源浏览、搜索、详情、选集、播放、错误重试；0.2.16 全站源版首页显示红果 / 黄豆 / 黄剧 / 黄果四个站源组，底层保留六个接口供独立管理 |
...
本轮完成变更 Dart / Go 源码格式整理、源码镜像同步及红果鉴 / 真果鉴两版 Android ARM64 Release 构建。两包均为版本码 2022、Android 8.0+、target SDK 36、仅 ARM64、`debuggable=false`，各含 17 个 DEFLATE 原生库；包 CRC、SHA256、16 KiB ZIP / ELF 页对齐、v2 签名、应用名称 / 包名和两版 Dart / Go 核心差异均已核对。签名沿用与旧包一致的 Android Debug 证书，不是正式生产签名。

0.2.16 未运行静态分析、自动化测试、回归、设备 / 模拟器、真实站源或双端验证，也未请求站源图片。26 项已实现待验证的状态保持未勾选，实际运行结论等待集中验证。
